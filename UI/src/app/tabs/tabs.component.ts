...
ngOnInit() {
  ...
  this.http.get("http://localhost:8082/Settlement_System/rest/hello/getAllCorporateActions").subscribe(
    data=> {
  ...
  generateCSR()
  {
    this.percentage = 100;
    this.disable2 = false;
    this.http.get("http://localhost:8082/Settlement_System/rest/hello/costOfSettlementReport").subscribe(data=>{
      console.log(data);
    })
  }
  AddTransaction()
...
onRowEditSave(client:any,index: number) {
  console.log(client);
  this.http.post("http://localhost:8082/Settlement_System/rest/hello/updateTransaction",client).subscribe(data=>{
      console.log(data);
    });
  if ((client.quantity) > 0) {
    delete this.clonedClientTrades[client.quantity];
    ...
    //console.log("here in else");
      this.messageService.add({severity:'error', summary: 'Error', detail:'Quantity cannot be 0'});
  }
}
...
onRowDelete(client: any, index: number)
{
  ...
  this.http.post("http://localhost:8082/Settlement_System/rest/hello/deleteTransaction",{
